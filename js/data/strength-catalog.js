 ...
 *
 *  解説の出典（2026-08-06・ユーザー指定により1名に限定）:
 *    今古賀翔【トレーニング科学】 https://www.youtube.com/c/ShoFitnessch
 *      早稲田大学大学院スポーツ科学研究科修了（スポーツ科学修士）、EVERLIFT代表。
 *      論文をもとに、力学と関節構造からフォームを説明する。BIG3の解説が特に論理的。
 *
 *  リンクの設計:
 *    種目ごとに、彼のチャンネル内をその種目で検索したURLへ飛ばす。
 *      https://www.youtube.com/c/ShoFitnessch/search?query=<種目>
 *    videoId を入れればその動画へ直行する（分岐は videoUrl にある）。
 *
 *    ★動画IDを推測で埋めていない。この環境からは YouTube を取得できず（403）、
 *      検索結果からも投稿者を確認できないため。確認できない動画IDに
 *      発信者の名前を付けるのは、検索リンクより悪い結果になる。
 *
 *    ★チャンネル内検索は語が長いとヒットしないので、検索語は短く保つこと。
 *      「バーベルバックスクワット フォーム 解説」ではなく「スクワット」。
 *
...
(function () {
  ...
  const SOURCE = {
    ...
    search: (q) => `https://www.youtube.com/c/ShoFitnessch/search?query=${encodeURIComponent(q).replace(/%20/g, "+")}`,
    note: "早稲田大学大学院スポーツ科学研究科修了（スポーツ科学修士）・EVERLIFT代表。論文をもとに、力学と関節構造からフォームを説明する",
    strength: "フォームの力学的な理屈",
  };
  const COACHES = [SOURCE];

  /* --------------------------------------------- 種目ごとのYouTubeリンク */
  /* チャンネル内検索は語が長いと0件になる。種目の核だけを短く渡すこと。
   * q.sho（種目ごとに調整した語）→ q.default の順で使う。 */
  function videoQuery(ex) {
    if (!ex) return "";
    const q = ex.q || {};
    return q.sho || q.default || String(ex.name || "");
  }

  function videoUrl(ex) {
    if (!ex) return null;
    if (ex.videoId) return `https://www.youtube.com/watch?v=${ex.videoId}`;
    return SOURCE.search(videoQuery(ex));
  }
